...
%     end
    ...
    for l = 1:nl-1
        ...
        elseif strcmp(s.nonlinearity{l}, 'rectify')
            cnn.f{l} = @(x) (max(x,0));
            cnn.df{l} = @(x,y) (y > 0);       
        elseif strcmp(s.nonlinearity{l}, 'softmax')
            if l ~= nl - 1 || ~strcmp(s.error_fxn, 'xentropy')
                error('softmax only implemented for output units with a crossentropy error fxn');
            end
            
            cnn.f{l} = @(x) (exp(x) / sum(exp(x(:))));
            cnn.df{l} = @(x,y) ( 1 ); %jacobian necessary to do this generally
        else
    ...
    elseif strcmp(s.error_fxn, 'hinge')
        if strcmp(s.nonlinearity{end}, 'logsig')
    ...
    elseif strcmp(s.error_fxn, 'xentropy')
        if ~strcmp(s.nonlinearity{end}, 'softmax')
            error('cross entropy error only implemented for softmax output units');
        end
        cnn.Ef = @(x, t) (-t .* log(x));
        cnn.dEf = @(x, t) ( shiftdim((diag(x(:)) - x(:) * x(:)') * (-t(:) ./ x(:)), -2) );
    else
